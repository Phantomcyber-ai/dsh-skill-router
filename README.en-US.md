...
- **Compatible with `0.1.2-alpha.5`**: alpha.4 removed `Session.events` in favor of the on-demand `snapshotEvents()`/`eventAt()` API; first-message detection uses the new API and falls back to the `events` array on older kernels (v0.4.1, verified against alpha.1 / alpha.3 / alpha.5 kernels).
